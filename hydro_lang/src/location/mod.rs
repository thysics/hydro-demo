...
pub trait Location<'a>: dynamic::DynLocation {
    ...
    ///
    /// This is useful for triggering periodic activity based on timing.
    /// The actual wall-clock time is not exposed, making this operation
    /// deterministic under the `AtLeastOnce` retry semantics.
    ///
    /// To get wall-clock time within a tick, use [`Tick::current_tick_start`]
    /// which provides explicit access to the timestamp with a [`NonDet`] guard.
    fn source_interval(
        &self,
    ...
    ///
    /// This is useful for triggering periodic activity based on timing
    /// with a configurable startup delay.
    /// The actual wall-clock time is not exposed, making this operation
    /// deterministic under the `AtLeastOnce` retry semantics.
    ///
    /// To get wall-clock time within a tick, use [`Tick::current_tick_start`]
