...
pub trait Location<'a>: dynamic::DynLocation {
    ...
    /// increasing order.
    ///
    /// This stream is deterministic because it only emits `()` values without
    /// exposing the wall-clock time. If you need access to the current time,
    /// use [`Location::wall_clock_time`] explicitly.
    ///
    /// # Example
    /// ```rust
    /// # #[cfg(feature = "deploy")] {
    ...
    ///     .map(q!(|_| 42))
    /// # }, |mut stream| async move {
    /// // 42, 42, 42, ... (first one after 50ms, then every 100ms)
    /// # assert_eq!(stream.next().await.unwrap(), 42);
    /// # assert_eq!(stream.next().await.unwrap(), 42);
    /// # }));
    /// # }
    ...
    ///
    /// # Non-Determinism
    /// This is non-deterministic because the returned time depends on when
    /// the code is executed. Use this only when you explicitly need access
    /// to wall-clock time.
    ///
    /// # Example
    /// ```rust
    ...
    /// # tokio_test::block_on(hydro_lang::test_util::stream_transform_test(|process| {
    /// let tick = process.tick();
    /// tick.wall_clock_time(nondet!(/** tracking timing for debugging */))
    ///     .all_ticks()
    ///     .map(q!(|instant| format!("Current time: {:?}", instant)))
    /// # }, |mut stream| async move {
