...
pub trait Location<'a>: dynamic::DynLocation {
    ...
    ///
    /// # Determinism
    /// Because this stream emits `()` (rather than exposing timestamps), it is
    /// deterministic with respect to `AtLeastOnce` semantics. If you need access
    /// to wall-clock time, use [`Tick::tick_instant`] within a tick/slice.
    fn source_interval(
        &self,
