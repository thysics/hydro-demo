...
pub trait Location<'a>: dynamic::DynLocation {
    ...
    ///
    /// This is useful for triggering periodic computations or sampling data
    /// at regular intervals. The first tick completes immediately, and subsequent
    /// ticks occur at the specified interval.
    ///
    /// # Example
    /// ```rust
    /// # #[cfg(feature = "deploy")] {
    ...
    ///
    /// This is useful for triggering periodic computations with a delayed start.
    /// The first tick occurs after the specified delay, and subsequent ticks
    ...
    /// [`tokio::time::Instant`].
    ///
    /// This is useful when you need to access the actual wall-clock time,
    /// for example for logging, metrics, or time-based computations.
    ///
    /// # Non-Determinism
    /// Because this reads the OS wall-clock time, the returned value is
    /// non-deterministic and will vary between executions. Use this only
    /// when you explicitly need the wall-clock time.
    ///
    /// # Example
    /// ```rust
    /// # #[cfg(feature = "deploy")] {
    ...
    /// # tokio_test::block_on(hydro_lang::test_util::stream_transform_test(|process| {
    /// let tick = process.tick();
    /// let now = tick.current_tick_instant(nondet!(/** logging only */));
