...
pub trait Location<'a>: dynamic::DynLocation {
    ...
    ///
    /// This is useful for triggering periodic operations without revealing
    /// the wall-clock time. To access the actual wall-clock time, use
    /// [`Tick::wall_clock_time`] instead.
    ///
    /// # Non-Determinism
    /// The timing of when `()` values are emitted is non-deterministic because
    /// it depends on the OS timer. This affects which elements are batched together
    /// in a tick.
    fn source_interval(
        &self,
