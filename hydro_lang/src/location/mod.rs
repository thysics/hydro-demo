...
pub trait Location<'a>: dynamic::DynLocation {
    ...
    ///
    /// The first tick completes immediately, with subsequent ticks occurring
    /// at the specified interval. This is deterministic because it only emits
    /// `()` values without revealing the actual timestamps.
    ///
    /// To get the actual wall-clock time, use [`Location::wall_clock_time`]
    /// which requires acknowledging the non-determinism.
    fn source_interval(
        &self,
    ...
    /// Generates a stream with unit values `()` emitted at a fixed interval,
    /// with an initial delay before the first emission.
    ///
    /// This is deterministic because it only emits `()` values without
    /// revealing the actual timestamps.
    ///
    /// To get the actual wall-clock time, use [`Location::wall_clock_time`]
    /// which requires acknowledging the non-determinism.
    ...
    ///
    /// # Non-Determinism
    /// This is non-deterministic because the actual timestamp value will vary
    /// between executions. Use this when you explicitly need to read the
    /// wall-clock time (e.g., for timeouts or performance measurements).
    ///
    /// For deterministic interval-based triggers, prefer [`Location::source_interval`]
    /// which only emits `()` without revealing timestamps.
    fn wall_clock_time(
        &self,
        _nondet: NonDet,
