...
pub trait Location<'a>: dynamic::DynLocation {
    ...
    ///
    /// This is useful as a timer trigger for periodic operations. The first
    /// tick completes immediately, and missed ticks will be scheduled as
    /// soon as possible.
    ///
    /// To get the actual wall-clock time, use [`wall_clock_time`](Self::wall_clock_time)
    /// which requires a non-determinism guard.
    fn source_interval(
        &self,
    ...
    ///
    /// This is useful as a timer trigger for periodic operations where
    /// an initial delay is needed. Missed ticks will be scheduled as
    /// soon as possible.
    ///
    /// To get the actual wall-clock time, use [`wall_clock_time`](Self::wall_clock_time)
    /// which requires a non-determinism guard.
    ...
    /// Returns a singleton containing the current wall-clock time.
    ///
    /// The time value is captured once and remains constant. This is useful
    /// for getting a timestamp for initialization or one-time events.
    ///
    /// # Non-Determinism
    /// The wall-clock time is non-deterministic because it depends on
    /// when the program is executed.
    fn wall_clock_time(&self, _nondet: NonDet) -> Singleton<tokio::time::Instant, Self, Bounded>
    where
        Self: Sized + NoTick,
    {
