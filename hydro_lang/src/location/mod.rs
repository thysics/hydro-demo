...
pub trait Location<'a>: dynamic::DynLocation {
    ...
    ///
    /// This is useful for creating periodic triggers or heartbeats.
    /// The first element is emitted immediately, and subsequent elements
    /// are emitted at the specified interval.
    ///
    /// # Example
    /// ```rust
    /// # #[cfg(feature = "deploy")] {
    ...
    ///
    /// This is useful for creating periodic triggers or heartbeats that
    /// should start after a specified delay.
    ///
    /// # Example
    /// ```rust
    /// # #[cfg(feature = "deploy")] {
    ...
    /// # tokio_test::block_on(hydro_lang::test_util::stream_transform_test(|process| {
    /// let tick = process.tick();
    /// process.source_interval_delayed(q!(Duration::from_millis(50)), q!(Duration::from_millis(100)))
    ///     .batch(&tick, nondet!(/** test */))
    ///     .map(q!(|_| 42))
    ///     .all_ticks()
    /// # }, |mut stream| async move {
    /// // 42, 42, 42, ... (after initial 50ms delay)
    /// # assert_eq!(stream.next().await.unwrap(), 42);
    /// # }));
    /// # }
    /// ```
    ...
    /// Generates a stream with wall-clock timestamps emitted at a fixed interval.
    ///
    /// Each value is the current time as a [`tokio::time::Instant`].
    /// The clock source used is monotonic, so elements will be emitted in
    /// increasing order.
    ///
    /// # Non-Determinism
    /// Because this stream reveals wall-clock time, it will be non-deterministic
    /// as each timestamp will be arbitrary. Use [`Location::source_interval`] if
    /// you only need periodic triggers without the actual timestamps.
    fn source_wall_clock_interval(
        &self,
        interval: impl QuotedWithContext<'a, Duration, Self> + Copy + 'a,
        _nondet: NonDet,
    ...
    /// # Non-Determinism
    /// Because this stream reveals wall-clock time, it will be non-deterministic
    /// as each timestamp will be arbitrary. Use [`Location::source_interval_delayed`]
    /// if you only need periodic triggers without the actual timestamps.
    fn source_wall_clock_interval_delayed(
        &self,
        delay: impl QuotedWithContext<'a, Duration, Self> + Copy + 'a,
        interval: impl QuotedWithContext<'a, Duration, Self> + Copy + 'a,
        _nondet: NonDet,
