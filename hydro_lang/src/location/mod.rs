...
pub trait Location<'a>: dynamic::DynLocation {
    ...
    ///
    /// The first tick completes immediately, then subsequent ticks occur at the
    /// specified interval. Missed ticks will be scheduled as soon as possible.
    ///
    /// This is deterministic because with `AtLeastOnce` semantics, the number and
    /// timing of elements doesn't affect correctness - only that eventually at least
    /// one element arrives per interval. To read the actual wall-clock time (which is
    /// non-deterministic), use [`Location::wall_clock_time`].
    fn source_interval(
        &self,
    ...
    /// Generates a stream with unit values `()` emitted at a fixed interval,
    /// with an initial delay before the first tick.
    ///
    /// After the initial delay, ticks occur at the specified interval.
    /// Missed ticks will be scheduled as soon as possible.
    ///
    /// This is deterministic because with `AtLeastOnce` semantics, the number and
    /// timing of elements doesn't affect correctness - only that eventually at least
    /// one element arrives per interval. To read the actual wall-clock time (which is
    ...
    /// Returns the current wall-clock time as a [`std::time::SystemTime`].
    ///
    /// This provides access to the wall-clock time, which can be useful for logging,
    /// timestamping, or other time-sensitive operations.
    ///
    /// # Non-Determinism
    /// This is non-deterministic because the wall-clock time depends on when the
    /// code actually executes, which can vary based on system load, scheduling, and
    /// other external factors. If you only need periodic triggers without caring about
    /// the actual time, use [`Location::source_interval`] instead.
    ///
    /// # Example
    /// ```rust,ignore
    /// use hydro_lang::prelude::*;
    ...
    ///
    /// // Get wall-clock time for timestamping (non-deterministic)
    /// let timestamped = stream.map(q!(|value| {
    ...
    fn wall_clock_time(
        &self,
        _nondet: NonDet,
    ) -> Singleton<std::time::SystemTime, Self, Bounded>
    where
        Self: Sized,
    {
        self.singleton(q!(std::time::SystemTime::now()))
    }
