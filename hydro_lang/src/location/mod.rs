...
pub trait Location<'a>: dynamic::DynLocation {
    ...
    /// increasing order. This is useful for triggering periodic actions without
    /// exposing the wall-clock time.
    ///
    /// To access the wall-clock time within a tick, use [`Tick::wall_clock_time`]
    /// instead, which explicitly marks the non-determinism of reading wall-clock time.
    fn source_interval(
        &self,
